...
readonly res_basename="ci-caa-${GITHUB_RUN_ID}_${GITHUB_RUN_ATTEMPT}"
...
delete_s3_buckets() {
  ...
  read -r -a buckets <<< "$(aws s3api list-buckets --query "Buckets[?contains(Name, '${res_basename}')].Name" --output text)"
