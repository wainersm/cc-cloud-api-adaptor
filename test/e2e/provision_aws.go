...
func NewAWSProvisioner() (*AWSProvisioner, error) {
	...
	return &AWSProvisioner{
		...
		Image: &AMIImage{
			...
			ID:              "", // To be defined when the image is registered
...
func (a *AWSProvisioner) UploadPodvm(imagePath string, ctx context.Context, cfg *envconf.Config) error {
	...
	fmt.Println("Convert qcow2 image to raw")
	...
	fmt.Println("Create vmimport service role")
	...
	fmt.Printf("Upload image %s to S3 bucket %s\n", imageRawPath, a.Bucket.Name)
	// TODO: upload the raw image
	//if err = a.Bucket.uploadFile(imagePath); err != nil {
	if err = a.Bucket.uploadFile2(imagePath); err != nil {
		return err
	}

	fmt.Println("Import disk snapshot")
	if err = a.Image.importEBSSnapshot(a.Bucket); err != nil {
		return err
	}

	//TODO: Define image name based on disk name.
	fmt.Println("Register image")
...
func (b *S3Bucket) uploadFile2(filepath string) error {
	var (
		fileSize              int64
	...
	fmt.Printf("Completed multi-part upload")
